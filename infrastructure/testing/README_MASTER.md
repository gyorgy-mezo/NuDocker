...
| Stage | What It Tests | Duration | Cost | Resources |
|-------|---------------|----------|------|-----------|
| **Stage 0** | Prerequisites & cloud access | 10 min | €0 | Local only |
| **Stage 1** | Basic VM provisioning | 15 min | €0.50 | 1 VM (2 vCPU, 4GB) |
| **Stage 2** | Packer image build | 45 min | €2.50 | 1 VM (4 vCPU, 8GB, temp) |
| **Stage 3** | Single-node HTCondor | 30 min | €1.50 | 1 VM (4 vCPU, 8GB) |
| **Stage 4** | Multi-node cluster + NFS | 45 min | €3.00 | 2 VMs (6 vCPU, 12GB) |
| **Stage 5** | SLURM integration | N/A | N/A | See Stage 6-7 |
| **Stage 6** | Scaled cluster | 60 min | €4.00 | 3 VMs (10 vCPU, 20GB) |
| **Stage 7** | Production deployment | 90 min | €6.00 | 6 VMs (20 vCPU, 48GB) |

**Total Testing Cost**: ~€7.50 (Stages 1-4 only)

### Cloud Discovery Tools

| Tool | Purpose | Duration | Output |
|------|---------|----------|--------|
| `discover_cloud.sh` | Quick environment scan | 30 sec | 1 report file (~1000 lines) |
| `check_quota.sh` | Quota verification | 5 sec | Console output with pass/fail |
| `collect_cloud_resources.sh` | Comprehensive inventory | 60 sec | ~20 files + archive |
...
### Use Case 1: Infrastructure Testing

**Goal**: Validate cloud setup before production deployment

**Steps**:
1. Run `stage0/verify_prerequisites.sh`
2. Proceed through stages 1-4 sequentially
3. Each stage builds on previous (don't skip!)
4. Review test results in `results/` directory

**Outcome**: Confidence that infrastructure works correctly

### Use Case 2: HTCondor Cluster Deployment

**Goal**: Deploy working HTCondor cluster for distributed computing

**Steps**:
1. Complete testing stages 1-4
2. Use full infrastructure deployment (../terraform/, ../ansible/)
3. Scale to desired cluster size
4. Submit test HTCondor jobs

**Outcome**: Production HTCondor cluster

### Use Case 3: MESA Parameter Grid

**Goal**: Run stellar evolution simulations for research

**Steps**:
1. Follow QUICKSTART_FOR_PIGNATARI.md
2. Run `collect_cloud_resources.sh`
3. Get optimized cluster design (based on your quota)
4. Deploy production cluster
5. Submit MESA parameter grid
6. Analyze results

**Outcome**: Scientific data products (stellar yields, abundances)

### Use Case 4: Cost Estimation

**Goal**: Estimate costs before committing resources

**Steps**:
1. Run `check_quota.sh` to see available resources
2. Run `collect_cloud_resources.sh` for detailed analysis
3. Review cost estimates in generated reports
4. Adjust cluster size based on budget
...
- **Terraform** ≥ 1.0
- **Packer** (for custom image builds)
- **Ansible** ≥ 2.9 (for production deployment)
...
- **OpenStack account** with application credentials
- **Sufficient quota**:
  - Minimum (testing): 6 vCPUs, 12 GB RAM, 2 instances
  - Recommended (production): 20+ vCPUs, 48+ GB RAM, 6+ instances
- **SSH key pair** uploaded to cloud
- **Network access** (internal network + external network for floating IPs)
...
- (Optional) HTCondor basics
- (Optional) MESA stellar evolution code
...
**Total for Stages 1-4**: ~€7.50
- Each stage uses minimal resources
- Resources deleted immediately after testing
- No ongoing costs
...
| Cluster Size | vCPUs | RAM | Est. Cost/Month | Use Case |
|--------------|-------|-----|-----------------|----------|
| **Minimal** | 6 | 12 GB | €50-100 | Testing, development |
| **Small** | 20 | 40 GB | €100-200 | Small parameter grids (10-30 models) |
| **Medium** | 40 | 80 GB | €200-400 | Full grids (50-100 models) |
| **Large** | 80 | 160 GB | €400-800 | Large grids, production science |

**Cost Optimization Tips**:
- Use spot/preemptible instances (50-90% discount)
- Delete resources when not in use
- Schedule auto-shutdown (nights/weekends)
- Compress and archive results regularly
- Use minimal flavors that meet requirements
...
- [x] Cost and timeline estimation tools
