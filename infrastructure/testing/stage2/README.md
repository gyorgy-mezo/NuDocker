...
**Build VM runs for ~45 minutes**, then auto-deletes.

**Estimated cost**: €2.00-3.00 for one build

**To reduce costs**:
- Use smaller flavor if builds succeed (try "m1.small" with 2 vCPU first)
- Run during off-peak hours (no cost difference, but faster network)
- Keep the image for reuse in Stages 3-7 (avoid rebuilding)

**Image storage cost**: ~€0.10-0.20/month for 3-4 GB image
